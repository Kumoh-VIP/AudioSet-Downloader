...
def download(class_name, args):
    ...
    with open(new_csv) as dataset:
        ...
        for row in tqdm(reader):
            ...
            arguments.append((row[1], row[0], dst_dir))

    parmap.starmap(download_file, arguments, pm_pbar=True, pm_processes = 12)

    # pool = Pool(processes=8)
    # pool.starmap(download_file, )

def download_file(ss, url, dst_dir):
    formatted = 'm4a'
    output_path = f'-o "{dst_dir}/{url}_{ss}.{formatted}"'
    # && ffmpeg -i {dst_dir}/{url}_{ss}.m4a -ar 16000 {dst_dir}/{url}_{ss}.wav && rm {dst_dir}/{url}_{ss}.m4a

    os.system(f'youtube-dl -q -ci --external-downloader ffmpeg --external-downloader-args "-ss {ss} -t 10" --postprocessor-args "-ar {DEFAULT_FS}" -x --audio-format "{formatted}" --audio-quality 0 "https://www.youtube.com/watch?v={url}" {output_path}')
